...
public final class Gameboard {
    ...
    public func getFreePositions() -> [GameboardPosition] {
        ...
        for (columnIndex, columnValue) in positions.enumerated() {
            //print("Found \(item) at position \(index)")
            for (rowIndex, rowValue) in columnValue.enumerated() {
                if rowValue == nil {
                    let freePosition = GameboardPosition(column: columnIndex, row: rowIndex)
